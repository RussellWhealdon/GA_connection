...
def get_ga_summary_data():
    ...
    for row in response.rows:
        ...
        source = row.dimension_values[1].value  # Extract the source
        ...
        rows.append([date, source, active_users, sessions, bounce_rate])
    
    # Create a DataFrame with appropriate column names
    df = pd.DataFrame(rows, columns=['Date', 'Source', 'Active Users', 'Sessions', 'Bounce Rate'])
